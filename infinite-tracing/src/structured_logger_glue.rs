...
pub fn setup_structured_logger() {
    ...
    impl structured_logger::Writer for MyWriter {
        fn write_log(&self, entries: &BTreeMap<Key, Value>) -> Result<(), Error> {
            let level = entries.get(&Key::from_str("level")).map(|value| value.to_string()).unwrap_or(String::from("<MISSING LEVEL>"));
            // Add the log record as an event in the current local span
            Event::add_to_local_parent(level, || entries.iter()
                .filter(|(&ref k, _v)| k != &Key::from_str("level") && k != &Key::from_str("target"))
