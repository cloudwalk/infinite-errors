...
impl Parse for MacroArgs {
    ...
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        const INVALID_IDENTIFIER_COMBINATION: &str = "Invalid identifier combination: only one of `err`, `ok` and `ret` may be specified";
        const INVALID_SKIP_COMBINATION: &str = "Invalid `skip` / `skip_all` combination: only one them may specified -- and just once";
                    ...
                    match ident.to_string().as_str() {
                        "skip" => {
                            ...
                            let punc = Punctuated::<Meta, Token![,]>::parse_terminated(&content)
                                .map_err(|err| syn::Error::new(Span::call_site(), format!("While parsing `skip`: {:?}", err)))?;
...
mod tests {
    ...
    fn typical_usage() {
        ...
        assert_eq!(parsed_args.custom_params, vec![("a".to_string(), "b".to_string())]);
    }
    ...
    fn identifier_parsing_scenarios() {
        ...
        assert_eq!(parsed_args.log_return, ReturnLogOptions::LogErrOnly, "Unexpected `log_return`");

        let parsed_args: MacroArgs = parse_str(r#"skip_all, err"#).unwrap();
        assert_eq!(parsed_args.log_return, ReturnLogOptions::LogErrOnly, "Unexpected `log_return`");
        assert!(!parsed_args.log_parameters, "`skip_all` should simply cause the logging of parameters to be disabled");
        ...
        assert_eq!(parsed_args.log_return, ReturnLogOptions::Skip, "Unexpected `log_return`");
        assert!(!parsed_args.log_parameters);
        ...
        assert_eq!(parsed_args.log_return, ReturnLogOptions::LogNonFallible, "Unexpected `log_return`");
        assert!(parsed_args.log_parameters);

        let parsed_args: MacroArgs = parse_str(r#"ok"#).unwrap();
        assert_eq!(parsed_args.log_return, ReturnLogOptions::LogResult, "Unexpected `log_return`");
        assert!(parsed_args.log_parameters);
    ...
    fn invalid_identifier_combinations() {
        ...
        fn assert(params: &str) {
            ...
            assert!(parsed_args.is_err(), "Parsing the parameter combination '{params}' should not have succeeded");
    ...
    fn invalid_standalone_identifier() {
        ...
        assert_eq!(parsed_args.unwrap_err().to_string(), r#"Unknown identifier parameter 'nonexistingparam' -- known ones are: 'err', 'ok', 'ret', 'skip_all'"#);
    }
    ...
    fn invalid_func_identifier() {
        ...
        assert!(parsed_args.is_err());
        assert_eq!(parsed_args.unwrap_err().to_string(), r#"Unknown func parameter 'nonexistingfunc' -- known one is: 'skip'"#);
    }
