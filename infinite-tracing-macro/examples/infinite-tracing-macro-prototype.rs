...
use minitrace::prelude::{full_name, Span, SpanContext, SpanId, TraceId};
...
fn log_all_calls_unwanted_parameters<Any>(a: u64, b: u64, c: Any) -> Result<u64, Box<dyn std::error::Error>> {
    Ok(a + b)
}
...
async fn async_endpoint_function() {
    ...
    let reconstructed_trace_id = Uuid::from_str(&trace_id_string).unwrap_or_else(|_| Uuid::new_v4()).as_u128();
    debug_assert_eq!(reconstructed_trace_id, trace_id);

    // let root_span = Span::root(full_name!(), SpanContext::random());
    let root_span = Span::root(full_name!(), SpanContext::new(TraceId(trace_id), SpanId::default()));
    let _guard = root_span.set_local_parent();
